...
public class BulkLoadReducer extends Reducer<LongWritable, String, ImmutableBytesWritable, Put> {
    ...
    private void writeOneRow(String row, Long id, Context context)
        ...
        if (user1ScreenName != null) {
            put.addColumn(COLF_USER1, COL_USER_SCREEN_NAME, user1ScreenName.getBytes());
        }
        if (user1Description != null) {
            put.addColumn(COLF_USER1, COL_USER_DESC, user1Description.getBytes());
        }
        ...
        if (user2ScreenName != null) {
            put.addColumn(COLF_USER2, COL_USER_SCREEN_NAME, user2ScreenName.getBytes());
        }
        if (user2Description != null) {
            put.addColumn(COLF_USER2, COL_USER_DESC, user2Description.getBytes());
        }

        // tweet
        if (tweetText != null) {
            put.addColumn(COLF_TWEET, COL_TWEET_TEXT, tweetText.getBytes());
        }
