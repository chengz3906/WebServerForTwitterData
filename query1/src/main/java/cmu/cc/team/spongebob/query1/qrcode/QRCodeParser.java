...
public class QRCodeParser {
     ...
     */
    private final int[] POS_DETECTION_PATTERN_2_V1;
    private final int[] POS_DETECTION_PATTERN_2_V2;
    ...
    public QRCodeParser() {
        ...
        POS_DETECTION_PATTERN_2_V1 = loadPositionDetectionPattern2(21);
        POS_DETECTION_PATTERN_2_V2 = loadPositionDetectionPattern2(25);
    ...
    private boolean checkPosAlighV1(int rStart, int cStart, int[] searchSpace) {
        for (int i = 0; i < 7; i++) {
            int shiftedPosPat = POS_DETECTION_PATTERN_2_V1[i] >>> cStart;
            if ((shiftedPosPat & searchSpace[rStart + i]) != shiftedPosPat) {
                return false;
            }
        }
        return true;
    ...
    private boolean checkPosAlighV2(int rStart, int cStart, int[] searchSpace) {
        for (int i = 0; i < 7; i++) {
            int shiftedPosPat = POS_DETECTION_PATTERN_2_V2[i] >>> cStart;
            if ((shiftedPosPat & searchSpace[rStart + i]) != shiftedPosPat) {
                return false;
            }
        }
    ...
    private int[] loadPositionDetectionPattern2(int size) {
        ClassLoader classLoader = getClass().getClassLoader();
        int[] pattern = new int[7];

        File file;
        if (size == 21) {
            file = new File(classLoader.getResource("QR_position_detection_pattern_v1.txt").getFile());
        } else {
            file = new File(classLoader.getResource("QR_position_detection_pattern_v2.txt").getFile());
        }
